...
public class ItemMatchingServiceImpl implements ItemMatchingService {
    ...
    public Item findOrCreateItem(ParsedInvoiceItemDTO parsedItem, Project project) {
        ...
        // Strategy 2: If itemCode is present, try to find by name (assuming itemCode could be in name)
        ...
        newItem.setName(parsedItem.getItemCode() != null ? parsedItem.getItemCode() : extractShortName(parsedItem.getDescription()));
        newItem.setDescription(parsedItem.getDescription());
        newItem.setPrice(parsedItem.getUnitPrice() != null ? parsedItem.getUnitPrice().doubleValue() : 0.0);
